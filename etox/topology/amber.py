...
hemeTemplate=os.path.join(DATADIR,'HEM.mol2')
mdrun=os.path.join(GROMACSHOME, 'bin/mdrun')
grompp=os.path.join(GROMACSHOME, 'bin/grompp')
solventMols=[os.path.join(DATADIR,"solventAmber.itp")]

def createTopology(fileIn,wdir,fmtIn=None,suffOut='2MD'):

    currdir=os.getcwd()
    os.chdir(wdir)

    logging.debug('Topology creation through Antechamber/ACPYPE')
    logging.debug('AMBERHOME is: %s'%AMBERHOME)
    logging.debug('ACPYPE executable is: %s'%ACPYPE)  
    
    fileInNm, fileInExt=os.path.splitext(fileIn)
    if fmtIn is None:
        fmtIn=fileInExt
    #Read input file and convert to mol2 for ACPYPE execution   
    mol=pybel.readfile(fmtIn,fileIn).next()
    charge=mol.charge
    fileTemp="%s_2ac"%(fileInNm)
    mol.write(format='mol2',filename="%s.mol2"%fileTemp, overwrite=True)
    # Prepare system path definition
    tempEnv=os.environ.copy()
    amberBin=os.path.join(AMBERHOME,'bin')
    tempEnv['PATH']="%s:%s"%(tempEnv['PATH'],amberBin)
    tempEnv['AMBERHOME']=AMBERHOME 
    
    cmd =  [ACPYPE, '-i', "%s.mol2"%fileTemp, '-n', str(charge), '-k', '"scfconv=1.d-08"']
    logging.debug(' '.join(cmd))
    fileOut="%s_%s"%(fileInNm,suffOut)
    ...
    try:
        ...
        logging.error('Topology creation failed: %s'%e)
        return (False,e)
    
    logging.debug('Topology created with success')

    outputFiles={'itp':'%s.itp'%fileOut,
                 'pdb': '%s.pdb'%fileOut
                 }
    os.chdir(currdir)
