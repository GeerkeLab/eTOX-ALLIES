...
def correctGro(confGro,resIn,resOut):
    # resin and resout are dictionaries {'name': 'WAT','atoms':['O','H1','H2'] }

    prot=pybel.readfile('gro',confGro).next()
    for res in openbabel.OBResidueIter(prot.OBMol):
        if res.GetName()==resIn['name']:
            res.SetName(resOut['name'])
            for atm in openbabel.OBResidueAtomIter(res):
                atmName=res.GetAtomID(atm).lstrip()
                if atmName in resIn['atoms']:
                    idxAtmRes=resIn['atoms'].index(atmName)
                    res.SetAtomID(atm,resOut['atoms'][idxAtmRes])
            
    prot.write('gro',confGro,overwrite=True)
    return confGro
